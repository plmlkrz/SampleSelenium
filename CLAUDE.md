...
`src/test/java/com/sampleselenium/drills/` contains numbered interview-prep modules (d01–d08), each with a complete `Source*` file and a `Practice*` skeleton (disabled tests Peter re-types from memory). [STUDY_GUIDE.md](STUDY_GUIDE.md) is the map: module table, run commands, Infosys/Deloitte question coverage, and talk tracks. Cucumber features live in `src/test/resources/features/`. CI lives in `.github/workflows/selenium-tests.yml` and `Jenkinsfile` (both are also drill material). Don't "fix" the `@Disabled`/`enabled=false` practice stubs — being empty is their job.
